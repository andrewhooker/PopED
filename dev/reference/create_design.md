...
Create design variables to fully describe a design. If variables are
supplied then these variables are checked for consistency and, if
possible, changed to sizes that make sense if there are inconsistencies.
...
- groupsize:
  ...
  Vector defining the size of the different groups (number of
  individuals in each group).

- m:

  A number defining the number of groups. Computed from xt if not
  defined.

- x:

  A matrix defining the discrete design variables for the model Each row
  is a group.
...
- ni:
  ...
  Vector defining the number of samples for each group, computed as all
  elements of xt for each group by default.

- model_switch:

  Matrix defining which response a certain sampling time belongs to.
  Defaults to one for all elements of xt.

## Details

If a value (or a vector/list of values) is supplied that corresponds to
only one group and the design has multiple groups then all groups will
have the same value(s). If a matrix is expected then a list of lists can
be supplied instead, each list corresponding to a group.
