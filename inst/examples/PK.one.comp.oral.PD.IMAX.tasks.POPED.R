...
source("/Users/ahooker/Documents/_PROJECTS/PopED_in_R/poped_r/models/PK.one.comp.oral.PD.IMAX.POPED.R")
...
ofv_fim(FIM,poped.db,ofv_calc_type=4) # log of det
get_rse(FIM,poped.db)

## ED evaluate. with MC and LHS sampling
output <- evaluate.e.ofv.fim(poped.db)
output$E_ofv
output$E_fim

## ED evaluate. with MC and LHS sampling
output <- evaluate.e.ofv.fim(poped.db,ED_samp_size=100)
output$E_ofv
output$E_fim

## API evaluate. with MC 
output <- evaluate.e.ofv.fim(poped.db,ofv_calc_type=4)
output$E_ofv
output$E_fim

## ED evaluate. with MC and random sampling
output <- evaluate.e.ofv.fim(poped.db,bLHS=0)
output$E_ofv
output$E_fim
