...
d_val=getfulld(d[,2,drop=F],poped.db$covd)
docc_full = getfulld(poped.db$docc[,2],poped.db$covdocc)
...
Rprof()
mftot0(model_switch,groupsize,ni,xt,x,a,bpop_val,d_val,sigma,docc_full,globalStructure)
Rprof(NULL)
summaryRprof()
...
n <- 100
for(i in 1:n){
  times <- system.time(mftot0(model_switch,groupsize,ni,xt,x,a,bpop_val,d_val,sigma,docc_full,globalStructure))
  tot.time <- tot.time + times
}
avg.time <- tot.time/n
avg.time
...
## for(i in 1:n){
##     start.time <- proc.time()
##     mftot0.cmp(model_switch,groupsize,ni,xt,x,a,bpop_val,d_val,sigma,docc_full,globalStructure)
##     tot.time <- tot.time + proc.time() - start.time
## }
## avg.time <- tot.time/n
## avg.time
