...
mftot4 <- function(model_switch,groupsize,ni,xt,x,a,bpop,d,sigma,docc,globalStructure){
  m=size(ni,1)
  s=0
  for(i in 1:m){
     if((ni[i]!=0 && groupsize[i]!=0)){
       ...
       returnArgs <- mf5(t(model_switch[i,1:ni[i,drop=F],drop=F]),t(xt[i,1:ni[i,drop=F],drop=F]),x_i,a_i, bpop,d,sigma,docc,globalStructure) 
mf_tmp <- returnArgs[[1]]
