...
mfea <- function(poped.db,model_switch,ni,xt,x,a,bpopdescr,ddescr,maxxt,minxt,maxa,mina,fmf,dmf,
                 EAStepSize=poped.db$EAStepSize,ourzero=poped.db$ourzero,
  ...
  if((poped.db$EACriteria!=1)){
    stop(sprintf('The criteria that can be used is Modified Fedorov Exchange Algorithm (EACriteria = 1)'))
  }
  poped.db$parallelSettings$bParallelMFEA
  if((poped.db$parallelSettings$bParallelMFEA == 1)){
  ...
  rho=poped.db$EAConvergenceCriteria
  xt_current = xt
  a_current = a
  x_current = x
  bpop=bpopdescr[,2,drop=F]
  d=getfulld(ddescr[,2,drop=F],poped.db$covd)
  doccfull = getfulld(poped.db$docc[,2,drop=F],poped.db$covdocc)
  
  stepsize = EAStepSize
  numpoints = poped.db$EANumPoints
    ...
    if((it==0)){
      if((poped.db$d_switch)){
        if((iParallelN==1)){ #IF NOT PARALLEL MFEA
          returnArgs <-  mftot(model_switch,poped.db$groupsize,ni,xt_current,x_current,a_current,bpop,d,poped.db$sigma,doccfull,poped.db) 
          nfmf <- returnArgs[[1]]
          poped.db <- returnArgs[[2]]
          optofv=ofv_fim(nfmf,poped.db)
        } else { ##STORE DESIGN FOR PARALLEL EXEC.
          designsin = update_designinlist(designsin,poped.db$groupsize,ni,xt_current,x_current,a_current,-1,0)
        }
      } else {
        returnArgs <-  ed_mftot(model_switch,poped.db$groupsize,ni,xt_current,x_current,a_current,bpopdescr,ddescr,poped.db$covd,poped.db$sigma,poped.db$docc,poped.db) 
        nfmf <- returnArgs[[1]]
        optofv <- returnArgs[[2]]
                ...
                if((sum(sum(inters))!=0) ){#If we have a sample defined here (accord. to G)
                  ...
                  if((min_val!=max_val)){
                    ...
                    for(j in 1:ntt){
                      ...
                      if((poped.db$d_switch)){
                        if((iParallelN==1)){
                          returnArgs <-  mftot(model_switch,poped.db$groupsize,ni,xt_tmp,x_current,a_current,bpop,d,poped.db$sigma,doccfull,poped.db) 
                          nfmf <- returnArgs[[1]]
                          poped.db <- returnArgs[[2]]
                          nofv=ofv_fim(nfmf,poped.db)
                        } else {
                          if((p==1)){
                            designsin = update_designinlist(designsin,poped.db$groupsize,ni,xt_tmp,x_current,a_current,-1,0)
                          } else {
                            nfmf = designout[[des_num]]$FIM
                        ...
                        returnArgs <-  ed_mftot(model_switch,poped.db$groupsize,ni,xt_tmp,x_current,a_current,bpopdescr,ddescr,poped.db$covd,poped.db$sigma,poped.db$docc,poped.db) 
                        nfmf <- returnArgs[[1]]
                        nofv <- returnArgs[[2]]
                        poped.db <- returnArgs[[3]]
                      }
                      if((iParallelN==1 || (p==2 && des_num > 1))){
                        ...
                        if((delta>delta_max)){
                          ...
                          fmf = nfmf
                          graph_det(poped.db$bShowGraphs,poped.db$optsw,xt_tmp,a_current,x_current,ni,m,dmfvector,best_it,poped.db$Engine)
                          if((!isempty(poped.db$strIterationFileName))){
                            write_iterationfile('Modified Exchange Algorithm',best_it,xt_tmp,a_current,x_current,ni,poped.db$groupsize,fmf,dmf,poped.db)
                          }
                        }
                      }
                    }
          ...
          if((poped.db$bUseGrouped_x)){
            if((bFirstInGroup_x)){
              ...
              for(k in 1:max(max(poped.db$Gx))){
                ...
                if((sum(sum(inters))!=0) ){#If we have a discrete variabel defined here (accord. to Gx)
                  tt=get_discrete_val(inters,poped.db$discrete_x)
                  if((length(tt)>1)){
                    ntt = length(tt)
                    for(j in 1:ntt){
                      ti = tt[j]
                      ...
                      if((poped.db$d_switch)){
                        if((iParallelN==1)){
                          returnArgs <-  mftot(model_switch,poped.db$groupsize,ni,xt_current,x_tmp,a_current,bpop,d,poped.db$sigma,doccfull,poped.db) 
                          nfmf <- returnArgs[[1]]
                          poped.db <- returnArgs[[2]]
                          nofv=ofv_fim(nfmf,poped.db)
                        } else { #PARALLEL DESIGN STORAGE
                          if((p==1)){
                            designsin = update_designinlist(designsin,poped.db$groupsize,ni,xt_current,x_tmp,a_current,-1,0)
                          } else {
                            nfmf = designout[[des_num]]$FIM
                            nofv = designout[[des_num]]$ofv
                            des_num = des_num +1
                        ...
                        returnArgs <-  ed_mftot(model_switch,poped.db$groupsize,ni,xt_current,x_tmp,a_current,bpopdescr,ddescr,poped.db$covd,poped.db$sigma,poped.db$docc,poped.db) 
                        nfmf <- returnArgs[[1]]
                        nofv <- returnArgs[[2]]
                        poped.db <- returnArgs[[3]]
                      }
                      if((iParallelN==1 || (p==2 && des_num > 1))){
                        ...
                        if((delta>delta_max)){
                          ...
                          dmfvector[best_it]=dmf
                          fmf = nfmf
                          graph_det(poped.db$bShowGraphs,poped.db$optsw,xt_current,a_current,x_tmp,ni,m,dmfvector,best_it,poped.db$Engine)
                          if((!isempty(poped.db$strIterationFileName))){
                            write_iterationfile('Modified Exchange Algorithm',best_it,xt_current,a_current,x_tmp,ni,poped.db$groupsize,fmf,dmf,poped.db)
                          }
                        }
                      }
                    }
            ...
            for(ct in 1:length(x[i,]) ){#For all discrete variables in group
              if((previous_type!=3 || previous_index_i!=i || previous_index_ct!=ct)){
                discrete_val = poped.db$discrete_x[[i,ct]]
                ...
                if((sum(sum(inters))!=0) ){#If we have a covariate defined here (accord. to Ga)
                  ...
                  if((min_val!=max_val)){
                    ...
                    for(j in 1:ntt){
                      ...
                      if((poped.db$d_switch)){
                        if((iParallelN==1)){
                          returnArgs <-  mftot(model_switch,poped.db$groupsize,ni,xt_current,x_current,a_tmp,bpop,d,poped.db$sigma,doccfull,poped.db) 
                          nfmf <- returnArgs[[1]]
                          poped.db <- returnArgs[[2]]
                          nofv=ofv_fim(nfmf,poped.db)
                        } else { #PARALLEL DESIGN STORAGE
                          if((p==1)){
                            designsin = update_designinlist(designsin,poped.db$groupsize,ni,xt_current,x_current,a_tmp,-1,0)
                          } else {
                            nfmf = designout[[des_num]]$FIM
                            nofv = designout[[des_num]]$ofv
                            des_num = des_num +1
                      ...
                      if((iParallelN==1 || (p==2 && des_num > 1))){
                        ...
                        if((delta>delta_max)){
                          ...
                          dmfvector[best_it]=dmf
                          fmf = nfmf
                          graph_det(poped.db$bShowGraphs,poped.db$optsw,xt_current,a_tmp,x_current,ni,m,dmfvector,best_it,poped.db$Engine)
                          if((!isempty(poped.db$strIterationFileName))){
                            write_iterationfile('Modified Exchange Algorithm',best_it,xt_current,a_tmp,x_current,ni,poped.db$groupsize,fmf,dmf,poped.db)
                          }
                        }
                      }
                    }
