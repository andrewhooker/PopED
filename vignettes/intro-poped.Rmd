...
We can also compare for instance if it is better to have more samples per subject or more subjects in the study - keeping the total number of observations fixed (5 samples/subjects * 20 subjects/group = 100 observations).
We thus change the group size to 25 and draw blood samples at only 4 time points: xt = c(1,6,240,245).
...
Now the precision on CL (bpop[3]) is better with the alternative design but the other parameters are less well estimated. So which of these designs is better?

It is possible to compare the objective function value (ofv) and see that for this dummy example the alternative design (more subjects per group, less samples per subject) the ofv is higher (=better).
