...
##2.	PKPD model (analytical equations) & multiple arms (different doses, sampling times)
...
library(deSolve)
library(Rcpp)# for compiled code
library(PopED)
```
...
```{r}
# cppFunction('List tmdd_qss_one_target_model_ode(double Time, NumericVector A, NumericVector Pars) {
#             int n = A.size();
#             NumericVector dA(n);
#             
#             double CL = Pars[0];
...
#     
#             dA[0] = -KA*A[0];
#             dA[1] = FAVAIL*KA*A[0]+(Q/V2)*A[2]-(CL/V1+Q/V1)*CFREE*V1-RTOT*KINT*CFREE*V1/(KSSS+CFREE);
...
For Ds optimality we add in the create.poped.database function the "ds_index" to indicate whether a paramter is interesting (=1) or not (=0). Moreover we need to set the "ofv_calc_type" to 6.
