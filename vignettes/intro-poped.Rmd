...
```{r, cache=TRUE, eval=T, fig.width=6}
plot_model_prediction(poped.db, model_num_points = 500)
```

Next, we plot the model typical values prediction intervals taking into account the between-subject variability (you can even investigate the effects of the resuldual, unexplained, variability with the `DV=T` argument): 
```{r simulate with BSV, cache=TRUE, eval=T, fig.width=6}
plot_model_prediction(poped.db, model_num_points=500, IPRED=T)
```

We can get these predictions nemerically as well:
```{r}
...
### Design optimization
Now we can optimize the sample times of the design
```{r optimize,cache=TRUE,message = FALSE,results='hide'}
...
Of course, this means that there are multiple samples at some of these time points.  We can explore a more practical design by looking at the loss of efficiency if we spread out sample times in a uniform distribution around these optimial points ($\pm 30$ minutes).  
...
```{r optimize dose,cache=TRUE,message = FALSE,results='hide', eval=FALSE}
output_dose_opt <- poped_optim(output$poped.db, opt_xt=T, opt_a=T, parallel = T)
```
...
```{r cost optimization, cache=TRUE, message = FALSE,results='hide'}
