...
The design and design space for adults is defined below (Two arms, 5 sample time points per arm, doses of 20 and 40 mg, `isPediatric = 0`). As we want to pool the results (i.e. add the FIMs together), we also have to provide  the `pedCL` parameter so that both the adult and childrems FIM have the same dimensions.
```{r}
poped.db <- 
  create.poped.database(
...
For Ds optimality we add the `ds_index` option to the `create.poped.database` function to indicate whether a paramter is interesting (=0) or not (=1). Moreover, we set the `ofv_calc_type=6` for computing the Ds optimality criterion (it is set to 4 by default, for computing the log of the determiant of the FIM). More details are available by running the command `?create.poped.database`.
