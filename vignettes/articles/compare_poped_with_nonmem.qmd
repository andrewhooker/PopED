...
dat <- model_prediction(poped.db, dosing = dosing_1, DV=T,filename = "./data/temp.csv") |> 
  dplyr::select(-c(IPRED,PRED))
readr::write_csv(dat,file = "./data/warfarin.csv",na = ".")
dat[1:20,]
```
...
write_file(nm_mod,file="./nonmem_models/run1.mod")
nm_mod
```

Then we estimate the model from the data in NONMEM

```{zsh nonmem-first-model, filename="PsN command"}
#| output: false
#| cache: true

execute ./nonmem_models/run1.mod
```

```{zsh, filename="PsN command"}
sumo ./nonmem_models/run1.lst
```

For NONMEM, parameter RSE is:

```{r}
ext.val <- read_table("./nonmem_models/run1.ext",skip=1,show_col_types = FALSE)
